...
Thread abstract the concept of a worker - an entity that gets something done. The work done by a thread is packaged up in its `run` method. When you need to get som work done, you need both a worker and the work - the `Runnable` interface abstracts the concept of work and allows that work to be associates with a worker - the thread. The `Runnable` interface declares a single method:
```Java
public void run();
```
The `Thread` class itself implements the `Runnable`interface because a thread can also define a unit of work.

We have seen that `Thread` can be extended to provide specific computation for a thread, but this approach is awkward in many cases. First, class extension is single inheritance - if you extend a class to make it runnable in a thread, you cannot extend any other class, even if you need to. Also, if your class needs only to be runnable, inheriting all the overhead of `Thread` is more than you need.
...
Let's have a second example which is not easy to implement without threads. We want to have an application which prints out ascending integer numbers at their own line every second. At the same time, we want to read from the keyboard whether the user want to stop the application. We can read the keyboard by invoking the `Scanner.netxline()` method. But the problem is, that this invoke is blocking - i.e., it will stop the execution of the application when it is waiting the input. And at the same time we should print those integer numbers.

Now we have to different tasks to be run at the same time. One task is to write numbers periodically to the output, and another task is to read the input to check whether user has pressed the escape character. Remember that when you start the application, the system has created one thread for you, it is the thread who has invoked your `main` method. Therefore, you need to create only one thread by yourself. Our work is to periodically write ascending numbers to the output. It can be easily implemented as a class who implements the `Runnable` interface:
```Java
...
Thread interrupted
```
The text `Program terminated` appears before `Thread interrupted` because Java does not guarantee that the thread is cancelled immediately when you invoke the `interrupt` method. 
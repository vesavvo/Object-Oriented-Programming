...
> Immutable data is data that cannot be changed after it is created. Mutable data is data that can be changed after it is created.
...
> By program state, we mean the stored information that a program can access and modify during its execution. This information is typically stored in variables, data structures, and memory locations. The program state represents the current values of these variables and data structures at any given point in time during the program's execution.
...
The functional style of programming is declarative programming. In the imperative style of coding, we define what to do and how to do it. Whereas, in the declarative style of coding, we only specify what to do. Let’s understand this with an example. Given a list of number let’s find out the largest of odd numbers divided by two from the list using an imperative and declarative style of coding. First the imperative version:
...
The first issue with the above code is that we are mutating the variable `largest` again and again.
...
> Notice that Java streams are different from Input/Output streams. The former is used to process collections of objects in a functional way, while the latter is used to read/write data from/to a file, network, or other sources.
...
> [Recursion](https://en.wikipedia.org/wiki/Recursion) is a programming technique where a method calls itself to solve a smaller instance of the same problem until a base case is reached. Recursion is more discussed in the course 'Algorithms and Data Structures'.
```Java
/**

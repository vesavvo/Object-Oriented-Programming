...
a formal system in mathematical logic introduced in the 1930s by [Alonso Church](https://en.wikipedia.org/wiki/Alonzo_Church) for expressing computation based on
...
by computers (Davis, M., "Computability and Unsovability", Dover, 1982).
...
```
Because functional interface has only one method, it is possible to assign a lambda expression to it -
...
You must use the return keyword when lambda expression contains multiple statements.
```java
interface Addable {  
    int add(int a,int b);  
}  

...
Every project you see in IntelliJ IDEA is in your computer's file system or on a centralized server. If you want to look your files, right-click the project name in project explorer window on the left-side of IDEA screen. Then select menu item Open In/Show in Explorer.

The project directory src contains the source code files of the program. If the package of a class is library, that class is stored inside the src/libary folder of the source code directory. If you want to create a new package, right-click the src folder, and select New/Package. When you create a new class (file), IntelliJ IDEA will create the appropriate `package` statements ath the beginning of the generated file.
...
Let's create a small application where we need `final` modifier in method declaration. We want to create a base class for a general vechicle:
```Java
package vechicles;
...
```
Constructor invokes allways the `doubleSound` method. If the `doubleSound` were overwritten in a subclass, modularity of the application would suffer. Therefore, we have given the `final` keyword to the definition of the `doubleSound` method.

Now we use the `Vechicle` to create a `Tram` class:
```Java
package vechicles.ground;

import vechicles.Vechicle;

public class Tram extends Vechicle {
    private int units;
...
```Java
import vechicles.ground.Tram;
import vechicles.water.Sub;
...
classDiagram
    ...
    class Storage {
        ...
        +Storage()
        +storeMeasurement(float value, long time)
    }
    class Estimator {
        +public float getTrend(int N)
    }
```
...
public class Storage {
    protected class ProcessTemp {
        ...
        public ProcessTemp(float value, long time) {
            this.value           = value;
     ...
     */
    public void storeMeasurement(float value, long time) {
        measurements.add(new ProcessTemp(value, time));
    }
}
```

We notice that the collection, `measurements` who stores the given measurements has an access right modifier `protected`. We don't want the user of this class to be able to mess with the data storage of stored values (`private`), but like the user of this class to be able to obtain those measured values, thus it is `protected`.

`Estimator` class inherits the `Storage` class, therefore it is able to access the measured values storage - in order to be able to make the estimate of the trend.
```Java
/**
 * Linear regression for a first order equation y = bx + a
 * <p>
 * It need Storage class to hold the values the estimation to be done.
 *
 * @author Jarkko Vuori
 * @see    <a href="Linear regression">https://pub.towardsai.net/calculating-simple-linear-regression-and-linear-best-fit-an-in-depth-tutorial-with-math-and-python-804a0cb23660</a>
 */
...
public class Estimator extends Storage {
    ...
    public float getTrend(int N) {
        ...
        float sumx = 0, sumy = 0;
        float Sxx, Sxy, Syy;
        float a, b;    	                        // regression coefficients y = bx + a
        ...
        while (n < N && n < measurements.size()) {
            ...
            // calculate current sample value, time is referenced to the first measurement sample
            y = measurements.get(i).value;
            x = (float)(measurements.get(i).measurementTime - t) / (3600.0f * INMILLIS); // x-scale is 1 C/h
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        s = askValue(scanner);
        while (!s.equals("x")) {
            estimator.storeMeasurement(Float.valueOf(s), System.currentTimeMillis());
            s = askValue(scanner);
        }
...
Use class variables to track the number of `Television` objects created, and restrict their count to maximum of 5 (an exception is thrown if new `Television` can not be created). Create a `TelevisionFactoryTester` class to test this feature of `Television` class.

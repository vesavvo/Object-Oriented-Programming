...
If $N$ is the number of code lines in the application, $n$ is the average number of codes lines in a software module, then the number of interactions (&asymp; complexity $C$) can be given as:
$$C(N,n)=\frac{(\frac{N}{n})(\frac{N}{n}-1)}{2}=\frac{\frac{N^2}{n^2}-\frac{N}{n}}{2}$$
This can be drawn as:

![](images/SysComplexity.png)

From the picture we can see that by dividing source code to reasonable sized, relatively independent and encapsulated modules, we can keep the complexity manageable.
...
The type of error is stated at the beginning of the list, and the following line tells us where the error occurred. The line "at Main.erroronpurpose(Main.java:17)" says that the error occurred at line number 17 (in method erroronpurpose of the class Main in the Main.java file.
```text
  Main.erroronpurpose(Main.java:17)
```
The next line "Main.main(Main.java:23)" tells us that the previous method (erroronpurpose) was invoked at the line 23 in the Main.java file.

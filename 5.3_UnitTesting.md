...
When an error occurs in a program, the program typically prints something called a stack trace, i.e., the list of method calls that resulted in the error. If you catch an exception, you can use `printStackTrace`
```Java
catch (Exception e) {
    e.printStackTrace();
...
2. Then you need to create Test Sources Root directory (green folder named 'test' on the picture below). Follow the instructions carefully, most probably you are using IntelliJ IDEA build took (builder), then you need to manually create the test root (which holds the JUnit 5 test source files).

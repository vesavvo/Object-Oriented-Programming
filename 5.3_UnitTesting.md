...
Instead of testing, another possibility is to mathematically _proof_ that the algorithm/program is correct. It works in ever-increasing cases, but in general it is difficult. For example, in the following piece of code (in C programming language), is the variable `x` being used uninitialized or not?
```C
int x;
...
In that nobody knows today whether odd perfect numbers exists, it is unknown whether the `write(x)` statement will ever be executed. Neither humans nor machines have a straightforward way of deciding this.

In practice, `write(x)` may be treated as unreachable because it is known that an odd perfect number must be greater than 10<sup>300</sup>, which means that at least 10<sup>600</sup> iterations must be taken. (Zellers, A., “Why Programs Fail, A Guide to Systematic Debugging”, MK, 2009, p. 166).

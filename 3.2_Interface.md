...
In this example, we've created a custom `Comparator` named `ageComparator` that compares `Person` objects based on their ages. The `compare` method is defined to compare the ages of two persons, resulting in a sorted list based on ascending age.

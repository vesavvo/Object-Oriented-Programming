...
**Task 1: Universal CarDriver**

Modify your `CarDriver` class in earlier labs to use (fill tank, accelerate, decelerate) `Car`, `SportsCar`, and `Bus` in polymorphic manner.

<!--
**Task 2: CD/DVD/BlueRay Player**
Modify your CDPlayer class to accept CDs, DVDs and BluRay discs. Use polymorphism to make it possible to insert all three types of discs (one at a time) into the player (one at a time).
-->

**Task 2: Universal HomeAppliance**
Create an abstract `HomeAppliance` class that has at least the functionality related to on/off switch. Try to find other device families that inherit from `HomeAppliance` and from which you can inherit concrete home appliances, like TV and Radio. (Hint: you might think of common functionality such as volume control)

**Task 3: Passenger**
Create an interface Passengers which, when implemented gives any vehicle a capability to carry passengers. Implement Bus, Ferry and PassengerJet using the vehicles example from an earlier sessions. You may also recreate vehicles hierarchy.

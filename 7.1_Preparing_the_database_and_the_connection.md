...
### Assignment 1: A database for the Currency Converter application
...
Do not include the queries in the database script. Instead, write them in a separate file, e.g., `queries.sql`. You can run the queries from the command line with the command `mysql -u root -p < queries.sql`. You can also run the queries from the MariaDB command line client.

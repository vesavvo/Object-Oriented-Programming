...
[Object-oriented programming](https://en.wikipedia.org/wiki/Object-oriented_programming) (OOP) is a powerful and essential paradigm that can greatly enhance your programming skills and open up numerous opportunities in the world of software development. Here are some compelling reasons to dive into OOP:
...
```
In this simple implementation, we used a very powerful abstraction and re-using tool, called as a method (or a function).
...
Now we have an abstraction tool for program statements (a method), let's study the data variables then. When we look at our application (above program), we can see directly *how* the coordinates are represented; there is no abstraction there. We would like to know *what* is this group of coordinates instead. Therefore, a lot of improvements can be done. When do we have:
```Java
int x1, y1, z1;
int x2, y2, z2;
```
...
Now the variables for holding coordinates are inside the memory area that is referenced using the 
variable `p1`. How are we able to access those variables? In Java (as in many other programming languages), we use the dot notation, like
```Java
p1.x = 11;
```
...
There is also a special method, called constructor, that will be automatically called when the
...
Class is a static entity; this means that the state of the class does not change during the runtime of the application. On the other hand, the state of the object changes during the runtime of the application—every time when instance variables are written.
...
```
In this demo, we define a Person class with private properties name and age, a constructor to initialize these properties, and getter methods to access them. The introduce() method prints out a simple introduction of the person.
...
A user story is concise, informal description of a software feature or requirement from
the perspective of an end user or customer. It is typically used in agile software development
methodologies, such as [scrum](https://www.scrum.org/resources/what-scrum-module), as a means to capture and communicate the desired functionality in a user-centered way.

...
> [XML](https://en.wikipedia.org/wiki/XML) (eXtensible Markup Language) is a markup language designed to store, transport, and organize data in a format that is both human-readable and machine-readable. It is a flexible text format derived from SGML (Standard Generalized Markup Language) and is widely used for data interchange between systems, especially in web development, configuration files, and document storage. Other markup languages include JSON (JavaScript Object Notation) and YAML (YAML Ain't Markup Language).

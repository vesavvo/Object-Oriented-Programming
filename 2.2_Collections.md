...
(index) where the key-value pair will be stored. Hash code function, `hashCode()`, for the Java String is calculated by evaluating
...
length of the string. Therefore `hasCode()` function returns different value for different strings with very high probability. For a good tutorial on hash codes, look [this](https://samwho.dev/hashing/).
```Java
class Main {
    public static void main(String[] args) {
        String str1 = "Java";

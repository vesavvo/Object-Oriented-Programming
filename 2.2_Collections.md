...
It would be nice to be able to handle a bunch of dogs together as a collection—insert new dogs easily into the collection, remove dogs from the collection, not need to decide on the maximum number of dogs etc.
...
`TreeSet` is a class in the Java Standard Library that implements the `Set` interface. It is a part of the Java Collections Framework and is used to store a collection of elements in a sorted order. Specifically, a `TreeSet` maintains its elements in a [red-black tree](https://en.wikipedia.org/wiki/Red%E2%80%93black_tree) data structure, which ensures that the elements are stored in a sorted order and allows for efficient operations like insertion, deletion, and searching.
...
Remember that if you need a collection that maintains sorted elements and allows duplicates, you can use the `SortedSet` interface with implementations like `TreeSet`. For collections without duplicates, you can explore implementations of the more general `Set` interface, such as `HashSet` or `LinkedHashSet`.

## Assignments

**Task 1: CD disk and disk collection**

Create an application that implements a CD disc class and a `CDCollection` class.  A CD contains at least artist name, album name, # of tracks, purchase date, and total length in seconds. `CDCollection` contains at least owner’s name and the actual CDs in the collection. In your application, initialize the collection with some fixed values (at least 10 CDs). Use `ArrayList` as your data structure.

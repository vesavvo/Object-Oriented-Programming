...
to decide on collection size at creation time (as it is with arrays). ArrayList size can grow automatically when new elements are added to the list.
...
```
Now we have variable persons which refer to an ArrayList whose elements are of class Person. Note that the Java compiler can deduct the type of the ArrayList from the lest side of the assignment operator, therefore the type on the right side of the assignment can be left empty, i.e., <>.
...
Let's have an example where we don't know at the compilation time a how large array we need to store information. In this case, we don't know how many names the user will enter to the system. Therefore, we need to use ArrayList collection class where the container can enlarge itself automatically when needed.
```Java
...
Java [HashMap](https://docs.oracle.com/javase/7/docs/api/java/util/HashMap.html) is a class that implements the [Map interface](https://docs.oracle.com/javase/7/docs/api/java/util/Map.html) and provides an efficient way to store and retrieve key-value pairs. In these pairs, each element is associated with a unique key that is used to retrieve the corresponding value. The HashMap uses the hash code (an integer in Java) of the key to determine the bucket (index) where the key-value pair will be stored. Hash code function, `hashCode()`, for the Java String is calculated by evaluating the following equation _hasCode = s[0]*31<sup>(n-1)</sup> + s[1]*31<sup>(n-2)</sup> + ... + s[n-1]_
...
Map interface method entrySet() returns a set, which has Map.Entry interface entries (key/value pairs). These key and value values can be obtained using `getKey()` and `getValue()` methods of the entry.   
...
**Task 1: CD disk and disk collection**

Create an application that implements a CD disc class and a `CDCollection` class.  A CD contains at least artist name, album name, # of tracks, purchase date, and total length in seconds. `CDCollection` contains at least owner’s name and the actual CDs in the collection. In you application initialize the collection with some fixed values (at least 10 CDs). Use `ArrayList` as your data structure.

**Task 2: Sorting the collection**

Implement the capability to show the CDs in the collection sorted by a) album name, b) number of tracks, and c) purchase date. Ask user how he/she wants the collection to be sorted.

**Task 3: TreeSet**

Implement task 2, but this time use `TreeSet` for storing the collection instead of an ArrayList

**Task 4: HashMap**

Implement task 2, but this time use `HashMap` for storing the collection instead of an ArrayList

...
From the code example above, you see that there is a reference to an external CSS file. The CSS file is named style.css, and it is located in the same folder as the Java source code file. The following code shows the content of the CSS file:
...
### Assignment 1: A currency converter
...
4. The user interface should include the following components:
   ...
   - Two choice boxes or combo boxes for selecting the source currency and the target currency. The labels should appear right on top of each choice box.

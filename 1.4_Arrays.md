...
a) Generate a random index `firstNameIndex` from the `firstNames` array.
b) Generate a random index `lastNameIndex` from the lastNames array.
c) Combine the randomly chosen first name and last name to form a new name.
d) Print the generated name.

...
flowchart TD
    ...
    SyncFunc["== synchronized deposit() (locked) =="]

    %% Säikeiden omat polut
    subgraph AccountA [Account A - Thread A]
        A1 --> A2 --> SyncStartA
    end

    subgraph AccountB [Account B - Thread B]
        B1 --> B2 --> SyncStartB
    end

    %% Synkronoitua deposit-funktiota varten
    SyncStartA & SyncStartB --> SyncFunc

    %% Jatkuu omilla poluilla depositin jälkeen
    SyncFunc --> A3 --> A4
    SyncFunc --> B3 --> B4
```

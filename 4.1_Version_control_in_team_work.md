...
In the previous section, we created a new branch called `feature` and added a new method to the `GreetingMachine` class. Now, we want to merge the `feature` branch to the `master` branch. This means that we want to add the new method to the `master` branch. We can do this in IntelliJ IDEA:
...
2.  Swith to the `master` branch. Then click **Git / Merge** in the main menu. Select the `feature` branch in the opening dialog box. Click **Merge**.  IntelliJ IDEA performs the merge operation. If there are no conflicts, the merge is successful.
...
In this situation, when both developers attempt to push their changes to the remote repository (origin), Git detects that the changes conflict with each other. This happens because Git cannot automatically determine which changes should be accepted and which should be rejected.

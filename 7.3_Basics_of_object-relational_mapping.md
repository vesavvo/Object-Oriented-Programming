...
- `jakarta.persistence.schema-generation.database.action`. The action to be performed when the database schema is created. In our case, we specify `drop-and-create`, which means that the schema will be dropped and recreated every time the application is started. If we wanted to use an existing database instead, we would specify `none`.
...
Later in this module, we discuss the handling of transactions and database exceptions in more detail.
...
Hibernate: create table employee (id integer not null auto_increment, email varchar(255), first_name varchar(255), last_name varchar(255), salary float(53) not null, primary key (id)) engine=InnoDB
...
The only real output of the application is the first and last name of the employee. The rest of the output is generated by Hibernate. The output shows that the database table is created, and the two employees are persisted to the database.

Next, it might be expected that a SELECT statement was generated for retrieving the first employee's data from the database. However,
this is not necessary, as the Employee object is managed by the `EntityManager` object. The `EntityManager` knows the state of the object and it knows that the object's status in memory matches the status in the database. Thus, the name of the first employee is immediately printed to the console.
